...
class EntryPoint(object):
    default_app_name = "calico"
    package_path = "/tmp/calico_deploy"
    package_name = ""
    configure_dir = ""
    system_service_dir = ""
    service_temp_name = "calico.yaml.temp"
    ...
    def begin(self):
        ...
        fill_service_configure(self.service_file_temp,
                               "{0}/calico.yaml".format(self.service_temp_path),
                               self.user_params)

        # Kubectl create from yaml file
        status, result = commands.getstatusoutput("kubectl create -f {0}/calico.yaml".format(self.service_temp_path))
        if status:
