...
class EntryPoint(object):
    default_app_name = "kubedns"
    package_path = "/tmp/kubedns_deploy"
    package_name = ""
    configure_dir = ""
    system_service_dir = ""
    service_temp_name = ""
    ...
    def begin(self):
        ...
        fill_service_configure("{0}/skydns-rc.yaml.temp".format(self.service_temp_path),
                               "{0}/skydns-rc.yaml".format(self.service_temp_path),
                               self.user_params)
        fill_service_configure("{0}/skydns-svc.yaml.temp".format(self.service_temp_path),
                               "{0}/skydns-svc.yaml".format(self.service_temp_path),
                               self.user_params)

        # Kubectl create from yaml file
        status, result = commands.getstatusoutput("kubectl create -f {0}/skydns-rc.yaml".format(self.service_temp_path))
        if status:
            raise Exception, result

        status, result = commands.getstatusoutput("kubectl create -f {0}/skydns-svc.yaml".format(self.service_temp_path))
        if status:
            raise Exception, result
