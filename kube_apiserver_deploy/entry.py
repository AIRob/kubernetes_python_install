...
class EntryPoint(object):
    default_app_name = "kube-apiserver"
    package_path = "/tmp/kubernetes_deploy"
    package_name = "kubernetes/server"
    configure_dir = "/etc/kube-apiserver/"
    system_service_dir = "/etc/systemd/system/"
    service_temp_name = "kube-apiserver.service.temp"
    ...
    def prepare(self):
        # Copy binary file
        status, result = commands.getstatusoutput("cp {0}/{1}/bin/kube-apiserver /usr/bin".format(self.package_path,
                                                                                     self.package_name))
        if status:
