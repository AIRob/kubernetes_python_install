...
class EntryPoint(object):
    default_app_name = "kubelet"
    package_path = "/tmp/kubernetes_deploy"
    package_name = "kubernetes/server"
    configure_dir = "/etc/kubelet/"
    system_service_dir = "/etc/systemd/system/"
    service_temp_name = "kubelet.service.temp"
    ...
    def prepare(self):
        # Copy binary file
        status, result = commands.getstatusoutput("cp {0}/{1}/bin/kubelet /usr/bin".format(self.package_path,
                                                                                     self.package_name))
        if status:
